...
// The sampler lives in lib/sight.mjs so the placement tool scores candidates with the
// exact formula the tuner solves against.
...
for (const p of list) {
  ...
  const fittedArea = (await sample(page, paintAt(p, ceilingFor, image), geo.targetPx, 0, true, fov)).changed;
  let scanned;
  let strength = 0;
  if (solve) {
    let lo = 0.004;
    ...
    for (let i = 0; i < 12; i++) {
      ...
      scanned = await sample(page, paint, geo.targetPx, fittedArea, true, fov);
  ...
  await frame(page, 'fitted', geo);
  if (raised || dimmed) scanned = await sample(page, paint, geo.targetPx, fittedArea, true, fov);
