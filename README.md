...
Nineteen paintings therefore give nineteen weeks -- one hundred and thirty-three days -- and `week.test.ts`
...
A browser that will not save the choice -- website data blocked -- still has it honored
for as long as the page is open, and the switch says the choice will not outlast that;
before this, a refused write read back as "never asked" and the beacons kept going out
under a switch that said off.
...
says so under "How it works" (a first-time player's panel opens short, with only the goal
and the gestures, so the full rules sit one tap behind it). Each one is credited by title, painter and year on the result card and in the
