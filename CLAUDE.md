...
- `?test` is the other one, and is not the same thing: it plays today's real puzzle and
  ...
  -- the parts practice mode cannot reach, because practice mode writes nothing. It lasts
  exactly as long as the URL says so: the plain address is always the real game. See
  "`?test`" in README.md.
